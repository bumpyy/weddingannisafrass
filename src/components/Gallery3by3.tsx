...
export default function Gallery() {
  ...
  return (
    <section className="container relative py-16">
      <h2 className="mb-8 text-lg italic text-center font-noto-serif-display">
        Our Gallery
      </h2>
        ...
        {photos.map((photo) => (
          <Dialog key={photo.name}>
            <DialogTrigger>
              <div
                key={photo.name}
                className="overflow-hidden transition-transform duration-700 rounded-md cursor-pointer aspect-square hover:scale-110"
              >
                <img
